...
func (s *Server) challengeFor(r *http.Request, difficulty int) string {
	...
	challengeData := fmt.Sprintf(
		"Accept-Language=%s,Ali-Cdn-Real-Ip=%s,User-Agent=%s,WeekTime=%s,Fingerprint=%x,Difficulty=%d",
		r.Header.Get("Accept-Language"),
		r.Header.Get("Ali-Cdn-Real-Ip"),
		r.UserAgent(),
...
func (s *Server) maybeReverseProxy(w http.ResponseWriter, r *http.Request, httpStatusOnly bool) {
	...
	if err != nil {
		...
		s.respondWithError(w, r, "Internal Server Error: administrator has misconfigured sisyphus. Please contact the administrator and ask them to look for the logs around \"maybeReverseProxy\"")
		return
	}

	r.Header.Add("X-sisyphus-Rule", cr.Name)
	r.Header.Add("X-sisyphus-Action", string(cr.Rule))
	...
	ckie, err := r.Cookie(sisyphus.CookieName)
	if err != nil {
	...
	r.Header.Add("X-sisyphus-Status", "PASS")
	s.ServeHTTPNext(w, r)
}
...
func (s *Server) checkRules(w http.ResponseWriter, r *http.Request, cr policy.CheckResult, lg *slog.Logger, rule *policy.Bot) bool {
	...
	default:
		...
		s.respondWithError(w, r, "Internal Server Error: administrator has misconfigured sisyphus. Please contact the administrator and ask them to look for the logs around \"maybeReverseProxy.Rules\"")
		return true
	}
	return false
}
...
func (s *Server) MakeChallenge(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		err := encoder.Encode(struct {
			...
			Error: "Internal Server Error: administrator has misconfigured sisyphus. Please contact the administrator and ask them to look for the logs around \"makeChallenge\"",
		})
		if err != nil {
...
func (s *Server) PassChallenge(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		lg.Error("check failed", "err", err)
		s.respondWithError(w, r, "Internal Server Error: administrator has misconfigured sisyphus. Please contact the administrator and ask them to look for the logs around \"passChallenge\".\"")
		return
	}
	lg = lg.With("check_result", cr)
	...
	cookiePath := "/"
	if sisyphus.BasePrefix != "" {
		cookiePath = strings.TrimSuffix(sisyphus.BasePrefix, "/") + "/"
	}
	// generate JWT cookie
