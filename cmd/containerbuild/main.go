...
var (
	...
	dockerRepo        = flag.String("docker-repo", "registry.int.xeserv.us/techaro/sisyphus", "Docker image repository for sisyphus")
...
func main() {
	...
	if *githubEventName == "pull_request" && *pullRequestID != -1 {
		*dockerRepo = fmt.Sprintf("ttl.sh/techaro/pr-%d/sisyphus", *pullRequestID)
		*dockerTags = fmt.Sprintf("ttl.sh/techaro/pr-%d/sisyphus:24h", *pullRequestID)
	...
	output, err := run(fmt.Sprintf("ko build --platform=all --base-import-paths --tags=%q --image-user=1000 --image-annotation=%q --image-label=%q ./cmd/sisyphus | tail -n1", strings.Join(tags, ","), *dockerAnnotations, *dockerLabels))
	if err != nil {
...
func parseImageList(imageList string) ([]image, error) {
	...
	for _, img := range images {
		...
		// reg.xeiaso.net/techaro/sisyphus:latest
		// repository: reg.xeiaso.net/techaro/sisyphus
		// tag:        latest
